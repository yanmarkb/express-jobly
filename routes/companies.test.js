...
describe("POST /companies", function () {
	...
	test("ok for admins", async function () {
		const resp = await request(app)
			.post("/companies")
			.send(newCompany)
			.set("authorization", `Bearer ${adminToken}`); //changed token to admin token
	...
	//Added this test to ensure that non-admin users cannot create a new company
	test("non-admin users can't create a new company", async function () {
		const resp = await request(app)
			.post("/companies")
			.send(newCompany)
...
describe("PATCH /companies/:handle", function () {
	//Changed to "works for admins" to ensure that only admins can update a company
...
//Added this test to ensure that non-admin users cannot update a company
test("non-admin users can't update a company", async function () {
	const resp = await request(app)
		.patch(`/companies/c1`)
		.send({
			name: "C1-new",
...
describe("DELETE /companies/:handle", function () {
	//Changed to "works for admins" to ensure that only admins can delete a company
	test("works for admins", async function () {
		const resp = await request(app)
			.delete(`/companies/c1`)
	...
	//Added this test to ensure that non-admin users cannot delete a company
	test("non-admin users can't delete a company", async function () {
		const resp = await request(app)
			.delete(`/companies/c1`)
			.set("authorization", `Bearer ${u1Token}`);
		expect(resp.statusCode).toEqual(401);
